...
export function SidebarRight() {
  ...
  const responseRoute = routes.find((route) => route.destinationId === selectedId)
  const routeResource = resources.find((resource) => resource.id === responseRoute?.resourceId)
  ...
  const confirmDispatch = () => { if (!dispatchCandidate || !selected) return; const result = dispatchResource(dispatchCandidate.resource.id, selected.id); setDispatchCandidate(null); setDispatchNotice(result.success ? `${dispatchCandidate.resource.callSign} dispatched. Calculating road route…` : result.error) }
